...
class ChatlyChannel(Document):
	...
	def on_trash(self):
		...
		frappe.db.delete("Chatly Channel Member", {"channel_id": self.name})

		# delete all messages when channel is deleted
		frappe.db.delete("Chatly Message", {"channel_id": self.name})

		# Delete the pinned channels
		frappe.db.delete("Chatly Pinned Channels", {"channel_id": self.name})
	...
	def after_insert(self):
		...
		if not frappe.flags.in_test:
			...
			if self.is_direct_message == 1:
				...
				chatly_users = self.channel_name.split(" _ ")
				unique_chatly_users = list(set(chatly_users))
				self.add_members(unique_chatly_users)
			else:
		...
		if not self.flags.is_created_by_bot:
			if self.type == "Private" or self.type == "Public":
				if (
					...
					and frappe.db.count("Chatly Channel Member", {"channel_id": self.name}) <= 1
				):
					pass
				elif frappe.db.exists(
					"Chatly Channel Member", {"channel_id": self.name, "user_id": frappe.session.user}
				):
					pass
				elif frappe.session.user == "Administrator":
					pass
	...
	def add_members(self, members, is_admin=0):
		# members is a list of Chatly User IDs
