...
class ChatlyWebhook(Document):
	...
	def before_insert(self):
		...
		webhook = frappe.get_all("Chatly Webhook", filters={"name": self.name})
		if webhook:
	...
	def get_doctype_and_event(self):
		doctypes_and_events = [
			...
			{"label": "Message Edited", "doctype": "Chatly Message", "event": "on_update"},
			{"label": "Message Deleted", "doctype": "Chatly Message", "event": "on_trash"},
			{"label": "Message Reacted On", "doctype": "Chatly Message Reaction", "event": "after_insert"},
			{"label": "Channel Created", "doctype": "Chatly Channel", "event": "after_insert"},
			{"label": "Channel Deleted", "doctype": "Chatly Channel", "event": "on_trash"},
			{
			...
			{"label": "User Added", "doctype": "Chatly User", "event": "after_insert"},
			{"label": "User Deleted", "doctype": "Chatly User", "event": "on_trash"},
		]
	...
	def get_conditions(self):
		...
		if self.trigger_webhook_on_condition:
			if self.conditions_on == "Channel":
				if doctype == "Chatly Channel":
					# return 'doc.name == self.channel_id'
					return f'doc.name == "{self.channel_id}"'
				elif doctype == "Chatly Channel Member":
					return f'doc.channel_id == "{self.channel_id}"'
				elif doctype == "Chatly Message":
					return f'doc.channel_id == "{self.channel_id}"'
				elif doctype == "Chatly Message Reaction":
					frappe.throw(_("Message Reaction cannot be triggered on Channel"))
				elif doctype == "Chatly User":
					frappe.throw(_("Chatly User cannot be triggered on Channel"))
