...
def create_poll(
	...
	"""
	Create a new poll in the Chatly Poll doctype.
	"""
	# Check if the current user has access to the channel to create a poll.
	if not frappe.has_permission(doctype="Chatly Channel", doc=channel_id, ptype="read"):
...
def get_poll(message_id):
	"""
	Get the poll data from the Chatly Poll doctype.
	...
	if not frappe.has_permission(doctype="Chatly Message", doc=message_id, ptype="read"):
		frappe.throw(_("You do not have permission to access this message"), frappe.PermissionError)

	poll_id = frappe.get_cached_value("Chatly Message", message_id, "poll_id")

	poll = frappe.get_cached_doc("Chatly Poll", poll_id)
...
def add_vote(message_id, option_id):
	...
	poll_id = frappe.get_cached_value("Chatly Message", message_id, "poll_id")
	is_poll_multi_choice = frappe.get_cached_value("Chatly Poll", poll_id, "is_multi_choice")

	if is_poll_multi_choice:
		for option in option_id:
			frappe.get_doc(
				{
					"doctype": "Chatly Poll Vote",
					"poll_id": poll_id,
					"option": option,
...
def retract_vote(poll_id):
	...
	votes = frappe.get_all(
		"Chatly Poll Vote", filters={"poll_id": poll_id, "user_id": user}, fields=["name"]
	)
	if not votes:
		frappe.throw(_("You have not voted for any option in this poll."))
	else:
		for vote in votes:
			frappe.delete_doc("Chatly Poll Vote", vote.name)
...
def get_all_votes(poll_id):
	...
	if not frappe.has_permission(doctype="Chatly Poll", doc=poll_id, ptype="read"):
		frappe.throw(_("You do not have permission to access this poll"), frappe.PermissionError)

	poll_doc = frappe.get_cached_doc("Chatly Poll", poll_id)
	...
	if poll_doc.is_anonymous:
		...
		votes = frappe.get_all(
			"Chatly Poll Vote", filters={"poll_id": poll_id}, fields=["name", "option", "user_id"]
		)
