...
from chatly.api.chat_stream import get_messages, get_newer_messages, get_older_messages
...
class TestChatStream(FrappeTestCase):
	def setUp(self):
		try:
			frappe.delete_doc("Chatly Channel", CHANNEL_ID)
		except frappe.exceptions.DoesNotExistError:
			pass
		frappe.clear_cache(doctype="Chatly Message")
		create_channel()
	...
	def test_get_messages_around_base_mid(self):
		...
		base_message_id = frappe.db.get_value(
			"Chatly Message", {"text": "Test Message 50", "channel_id": CHANNEL_ID}, "name"
		)
	...
	def test_get_messages_around_base_with_fewer_new_messages(self):
		...
		"""
		base_message_id = frappe.db.get_value(
			"Chatly Message", {"text": "Test Message 95", "channel_id": CHANNEL_ID}, "name"
		)
		response = get_messages(CHANNEL_ID, base_message=base_message_id)
	...
	def test_get_messages_around_base_with_fewer_old_messages(self):
		...
		"""
		base_message_id = frappe.db.get_value(
			"Chatly Message", {"text": "Test Message 5", "channel_id": CHANNEL_ID}, "name"
		)
		response = get_messages(CHANNEL_ID, base_message=base_message_id)
