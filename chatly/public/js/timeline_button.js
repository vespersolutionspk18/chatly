      ...
      refresh: function (frm) {
        if (!frm.is_new()) {
          if (frm.footer?.frm?.timeline && !buttonAdded) {
            ...
            let get_channels = () => {
              return frappe.call({
                method: 'chatly.api.chatly_channel.get_channels',
                args: {
                  hide_archived: true,
                },
                callback: function (r) {
            ...
            // check the button is not already added by checking the class name 'send-chatly-button'

            timeline.add_action_button(
              __('Send a Chatly'),
              send_chatly,
              'share',
              'btn-secondary send-chatly-button'
            );
            buttonAdded = true;
          }
        }
