...
const useUnreadCount = () => {
  const { data } = useSWRSubscription(
    "chatly.api.chatly_message.get_unread_count_for_channels",
    (key, { next }) => {
      //Initial load
      fetcher(key).then((data) => next(null, data));

      frappe.realtime.on("chatly:unread_channel_count_updated", (event) => {
        if (event.play_sound && event.sent_by !== frappe.session.user) {
          frappe.utils.play_sound("chatly_notification");
        }
        fetcher(key).then((data) => next(null, data));
      });

      return () => frappe.realtime.off("chatly:unread_channel_count_updated");
    },
  );
