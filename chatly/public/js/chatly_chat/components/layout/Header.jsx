...
const Header = ({
    ...
    return <div className="chatly-header">
        {selectedChannel ? <ChannelHeader channelID={selectedChannel} onBackClick={closeChannel} /> :
            <div style={{
                ...
                <span role='button' className="chatly-logo cal-sans" onClick={toggle}>chatly</span>

                <a href='/chatly' target='_blank' className='btn btn-xs open-chatly-button' style={{
                    lineHeight: '1.4rem',
                    // width: "24px",
                    display: 'flex',
                    alignItems: 'center',
        ...
        <div>
            {unreadMessageCount ? <span className="chatly-unread-count">{unreadMessageCount} unread</span> : null}
            <button className="btn btn-md back-button" onClick={toggleChatly}>
                {isOpen ?
...
const ChannelHeader = ({ channelID, onBackClick }) => {
    ...
    return <div className="chatly-channel-header">
        ...
        {channel?.is_direct_message ? <DMChannelHeader channel={channel} /> :
            <a href={`/chatly/channel/${channelID}`} target="_blank" title="Open channel in Chatly">
                <ChannelIcon channelType={channel?.channel_type} />
                <span className="chatly-channel-header-name cal-sans">{channel?.channel_name ?? channelID}</span>
            </a>
...
const DMChannelHeader = ({ channel }) => {
    ...
    return <a title="Open channel in Chatly" className="chatly-dm-channel-header" href={`/chatly/channel/${channel.name}`} target="_blank">
        <Avatar user={user} fallback={channel.peer_user_id} />
        <span className="chatly-channel-header-name cal-sans">{user?.full_name ?? user?.name}</span>
    </a>
