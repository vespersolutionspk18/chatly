...
const MessageHeader = ({ message, user }) => {
    ...
    return <div className='chatly-message-item-header'>
        <span className='chatly-message-item-header-name'>{user?.full_name}</span>
        <span className='chatly-message-item-header-time'>{moment(message.creation, frappe.defaultDatetimeFormat).format('HH:mm A')}</span>
    </div>
}
