...
const MessageStream = ({ channelID }) => {
  ...
  const { data } = useSWRSubscription(
    `chatly.api.chatly_message.get_messages_with_dates?channel_id=${channelID}`,
    (key, { next }) => {
      //Initial load
      fetcher(key).then((data) => next(null, data));

      if (channelID) {
        frappe.socketio.doc_subscribe("Chatly Channel", channelID);
        frappe.socketio.doc_open("Chatly Channel", channelID)
      ...
      return () => {
        ...
        frappe.socketio.doc_close("Chatly Channel", channelID);
        try {
          frappe.socketio.doc_unsubscribe("Chatly Channel", channelID);
        } catch (e) {
          // console.log(e)
        }
      }
  ...
  return (
    <div>
      ...
      <div className="chatly-message-stream-container" ref={containerRef}>
